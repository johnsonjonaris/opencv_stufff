...
public:
	...
	void ComputeHistogram( )
	{
		...
		for (int channel=0; (channel < mNumberChannels); channel++)
		{
			...
			int *mch = {0};
            calcHist(&(image_planes[channel]), 1, mChannelNumbers, Mat(),
                     mHistogram[channel], 1 , mNumberBins, &channel_ranges);
		}
	}
	void SmoothHistogram( )
	{
		for (int channel=0; (channel < mNumberChannels); channel++)
		{
			...
			{
                mHistogram[channel].at<float>(i) = (temp_histogram.at<float>(i-1) +
                                                    temp_histogram.at<float>(i) +
                                                    temp_histogram.at<float>(i+1)) / 3;
			}
		}
	}
...
void HistogramsDemos( Mat& dark_image, Mat& fruit_image, Mat& people_image, Mat& skin_image, Mat all_images[], int number_of_images )
{
	...
	for (int image_index=0; (image_index<number_of_images); image_index++)
	{
		if (image_index != index_of_reference_image)
		{
            ...
            double matching_score = compareHist(reference_histogram.getHistogram(),
                                                comparison_histogram.getHistogram(),
