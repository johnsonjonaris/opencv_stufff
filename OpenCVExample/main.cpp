...
/*
  Note that this program only works if the openCV is
...
//    for (int file_no=0; (file_no < number_of_images); file_no++)
//    {
//        imshow( "Display window", image[file_no] );     // Show our image inside it.
//        waitKey(0);                                     // Wait for a keystroke in the windo
//    }
    ...
    for (int video_file_no=0; (video_file_no < number_of_videos); video_file_no++)
	{
        ...
        if( !video[video_file_no].open(filename) )
        {
